...
const cardColorGradients = [
  { from: "#ffffff", to: "#1e293b", isNoColor: true },
...
const AddAccountModal = ({ open, onOpenChange, onAdd }: AddAccountModalProps) => {
  ...
  const getNetworkLabel = (network: "visa" | "mastercard" | "humo" | "uzcard") => {
  ...
  const handleAdd = () => {
    ...
    const formattedColor =
      newType === "card"
        ? newColor
          .replace(/from-(\#[a-f0-9]{6})/, "from-[$1]")
          .replace(/to-(\#[a-f0-9]{6})/, "to-[$1]")
        : newType === "bank"
    ...
    setNewCurrency("USD");
    setNewNetwork("visa");
    setNewColor(`from-${cardColorGradients[0].from} to-${cardColorGradients[0].to}`);
  ...
  const handleNetworkChange = (network: "visa" | "mastercard" | "humo" | "uzcard") => {
    ...
    const gradient = cardColorGradients.find(
      (g) =>
        ...
        <div className="space-y-3">
          ...
          {newType !== "card" && (
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {(["USD", "UZS", "EUR"] as const).map((currency) => (
                  <button
                    key={currency}
                    onClick={() => setNewCurrency(currency)}
                    className={`py-2 rounded-2xl text-xs font-semibold transition-colors shadow-sm ${newCurrency === currency ? "bg-primary text-white" : "btn-secondary"
                      }`}
                  >
                    {currency}
                  </button>
          ...
          {newType === "card" && (
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {(["visa", "mastercard", "humo", "uzcard"] as const).map((network) => (
                  <button
                    key={network}
                    onClick={() => handleNetworkChange(network)}
                    className={`py-2 rounded-2xl text-xs font-semibold capitalize transition-colors shadow-sm ${newNetwork === network ? "bg-primary text-white" : "btn-secondary"
                      }`}
                  >
                ...
                {cardColorGradients.map((gradient, index) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex-shrink-0 w-8 h-8 rounded-lg transition-all duration-200 hover:scale-110 ${newColor === colorKey
