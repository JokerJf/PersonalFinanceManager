...
const Transactions = () => {
  const { transactions, selectedTransactionId, setSelectedTransactionId, setAddTransactionModalOpen, accounts, isLoadingData, refreshData } = useApp();
  ...
  const accountFiltered = selectedAccountId === "all" ? filtered : filtered.filter(t => t.accountId === selectedAccountId);
  
  // Sort transactions by date (newest first)
  const sorted = [...accountFiltered].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
        ...
        <div className="flex gap-2">
          ...
          <div className="relative flex-1">
            ...
            >
              ...
              <ChevronDown size={18} className={`transition-transform shrink-0 ml-2 ${showFilters ? "rotate-180" : ""}`} />
            </button>
            ...
            {showFilters && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-[#0d1017] border border-gray-200 dark:border-slate-700 rounded-2xl shadow-lg z-10 overflow-hidden max-h-60 overflow-y-auto">
                <button
                  ...
                  className={`w-full text-left px-4 py-2.5 text-sm ${selectedAccountId === "all" ? "bg-primary/10 text-primary" : "text-slate-700 dark:text-white hover:bg-gray-100 dark:hover:bg-[#2a2f3c4d]"}`}
                >
                  All Accounts
                </button>
                {accounts.map(account => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-2.5 text-sm ${selectedAccountId === account.id ? "bg-primary/10 text-primary" : "text-slate-700 dark:text-white hover:bg-gray-100 dark:hover:bg-[#2a2f3c4d]"}`}
                  >
                    {account.name}
                  </button>
                ))}
          ...
          <div className="relative flex-1">
            ...
            >
              ...
              <ChevronDown size={18} className={`transition-transform shrink-0 ml-2 ${showTypeFilter ? "rotate-180" : ""}`} />
            </button>
              ...
              <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-[#0d1017] border border-gray-200 dark:border-slate-700 rounded-2xl shadow-lg z-10 overflow-hidden">
                {filters.map((f) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-2.5 text-sm capitalize ${filter === f.key ? "bg-primary/10 text-primary" : "text-slate-700 dark:text-white hover:bg-gray-100 dark:hover:bg-[#2a2f3c4d]"}`}
                  >
                    {f.label}
                  </button>
                ))}
