...
const Dashboard = () => {
  ...
  const quickActions = [
    { icon: ArrowUpRight, label: t("common.expense"), bgColor: "bg-destructive/10", color: "text-destructive", action: () => openModal("expense") },
    { icon: ArrowDownLeft, label: t("common.income"), bgColor: "bg-success/10", color: "text-success", action: () => openModal("income") },
    { icon: ArrowLeftRight, label: t("common.transfer"), bgColor: "bg-primary/10", color: "text-primary", action: () => openModal("transfer") },
    { icon: Plus, label: t("common.debt"), bgColor: "bg-warning/10", color: "text-warning", action: () => navigate("/debts") },
  ];
    ...
    <div className="space-y-6">
        ...
        <div className="relative">
          <div className="flex items-center justify-between mb-4 sm:mb-6">
            <div className="flex items-center gap-2">
              ...
              <p className="text-xs sm:text-sm font-medium text-slate-600 dark:text-white/80">{t("common.totalBalance")}</p>
            </div>
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            {balanceCurrency === "all" ? (
              <div className="flex items-center gap-1.5 group-hover:gap-2 transition-all">
                ...
                <span className="text-[10px] sm:text-xs text-slate-500 dark:text-white/70 font-medium">{t("common.tapToSwitch")}</span>
              </div>
            ) : (
              <div className="flex items-center gap-1.5">
                <div className="w-1.5 h-1.5 rounded-full bg-slate-400 dark:bg-white/40" />
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={() => navigate("/accounts")} className="text-[10px] sm:text-xs text-primary font-semibold">{t("common.seeAll")}</button>
        </div>
      ...
      <div>
        <h2 className="section-title mb-2 sm:mb-3">{t("dashboard.quickActions")}</h2>
      ...
      <div className="grid grid-cols-2 gap-2 sm:gap-3 exchange-ai-buttons">
        <button onClick={() => navigate("/exchange")} className="rounded-2xl border border-border/30 flex items-center gap-2 sm:gap-3 py-3 cursor-pointer active:scale-[0.98] transition-transform px-4 dark:bg-[rgba(28,32,44,0.3)] bg-white shadow-sm">
          ...
          <div className="text-left min-w-0">
            <p className="text-xs sm:text-sm font-semibold truncate">{t("common.exchangeRates")}</p>
            <p className="text-[8px] sm:text-[10px] text-muted-foreground font-medium truncate">{t("common.currencyConverter")}</p>
          </div>
        </button>
        <button onClick={() => navigate("/chatbot")} className="rounded-2xl border border-border/30 flex items-center gap-2 sm:gap-3 py-3 cursor-pointer active:scale-[0.98] transition-transform px-4 dark:bg-[rgba(28,32,44,0.3)] bg-white shadow-sm">
          ...
          <div className="text-left min-w-0">
            <p className="text-xs sm:text-sm font-semibold truncate">{t("common.aiAssistant")}</p>
            <p className="text-[8px] sm:text-[10px] text-muted-foreground font-medium truncate">{t("common.askAnything")}</p>
          </div>
        </button>
      </div>
      )}
      ...
      <div className="recent-transactions">
        <div className="flex items-center justify-between mb-2 sm:mb-3">
          ...
          <button onClick={() => navigate("/transactions")} className="text-[10px] sm:text-xs text-primary font-semibold">{t("common.seeAll")}</button>
        </div>
